...
ZizanieController.prototype = {
    ...
    _session: function() {
        ...
        return function(req, res, next) {
            ...
            // facebook session, whe must check if uid match a user
                ...
                that.db.model('User').findFacebookId(session.userId, function(err, result) {
                    if (result) {
    ...
    sign_in: function(req, res) {
        this.db.model('User').findUsername(req.body.username, function(err, result) {
    ...
    update_account_facebook: function(req, res) {
        ...
        if (fbSession) {
            this.db.model('User').findUsername(req.session.username, function(err, user) {
                if (user) {
    ...
    lost_password_send_token: function(req, res) {
        ...
        this.db.model('User').findUsername(username, function(err, user) {
            if (user) {
                var to = user.email;
